...
function Registro() {
  ...
  async function handleRegistro() {
    if (!nombre.trim() || !apellido.trim()) { setError('El nombre y apellido son obligatorios'); return }
  ...
  return (
    <div style={{
      ...
      <div style={{ background: 'white', padding: '40px', borderRadius: '16px', boxShadow: '0 20px 60px rgba(0,0,0,0.4)', width: '100%', maxWidth: '420px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '12px' }}>
          <div>
            <label style={{ fontSize: '13px', fontWeight: '500', color: '#555', display: 'block', marginBottom: '4px' }}>Nombre <span style={{ color: '#e74c3c' }}>*</span></label>
            <input placeholder="Ej: Juan" value={nombre} onChange={(e) => setNombre(e.target.value)} style={{ width: '100%', margin: 0 }} />
          </div>
          <div>
            <label style={{ fontSize: '13px', fontWeight: '500', color: '#555', display: 'block', marginBottom: '4px' }}>Apellido <span style={{ color: '#e74c3c' }}>*</span></label>
            <input placeholder="Ej: García" value={apellido} onChange={(e) => setApellido(e.target.value)} style={{ width: '100%', margin: 0 }} />
          </div>
        </div>

        <div style={{ marginBottom: '12px' }}>
          <label style={{ fontSize: '13px', fontWeight: '500', color: '#555', display: 'block', marginBottom: '4px' }}>Email</label>
          <input type="email" placeholder="tu@email.com" value={email} onChange={(e) => setEmail(e.target.value)} style={{ width: '100%' }} />
        </div>

        <div style={{ marginBottom: '16px' }}>
          <label style={{ fontSize: '13px', fontWeight: '500', color: '#555', display: 'block', marginBottom: '4px' }}>Contraseña</label>
          <input type="password" placeholder="Mínimo 6 caracteres" value={password} onChange={(e) => setPassword(e.target.value)} style={{ width: '100%' }} />
        </div>

        <div style={{ marginBottom: '16px' }}>
          <label style={{ fontSize: '13px', fontWeight: '500', color: '#555', display: 'block', marginBottom: '8px' }}>¿Qué tipo de cuenta querés?</label>
          <div style={{ display: 'flex', gap: '10px' }}>
            <div onClick={() => setRol('buscador')} style={{ flex: 1, padding: '12px', borderRadius: '10px', border: rol === 'buscador' ? '2px solid #f4a261' : '2px solid #eee', background: rol === 'buscador' ? '#fff8f3' : 'white', cursor: 'pointer', textAlign: 'center' }}>
            ...
            <div onClick={() => setRol('profesional')} style={{ flex: 1, padding: '12px', borderRadius: '10px', border: rol === 'profesional' ? '2px solid #f4a261' : '2px solid #eee', background: rol === 'profesional' ? '#fff8f3' : 'white', cursor: 'pointer', textAlign: 'center' }}>
              <div style={{ fontSize: '24px', marginBottom: '4px' }}>🔧</div>
